# Calibration - SQS-triggered ECS Fargate service (0/1 scaling)
resource "aws_ecs_task_definition" "calibration" {
  family                   = "${var.project_name}-${var.environment}-ai-calibration-task"
  network_mode             = "awsvpc"
  requires_compatibilities = ["FARGATE"]
  cpu                      = var.calibration_cpu
  memory                   = var.calibration_memory
  ...
  container_definitions = jsonencode([
    {
      name      = "calibration"
      image     = "${aws_ecr_repository.ai_services["calibration"].repository_url}:latest"
      essential = true

      command = [
        "uvicorn", "services.radiometric_calibration.app.__main__:app",
      ...
      environment = [
        ...
        { name = "CALIBRATION_SERVICE_PORT", value = "8001" },
      ...
      logConfiguration = {
        ...
        options = {
          ...
          "awslogs-stream-prefix" = "calibration"
        }
      }
    }
  ])
  ...
  tags = merge(var.tags, {
    Name      = "${var.project_name}-${var.environment}-ai-calibration-task"
    Component = "AI-Services"
  })
}

resource "aws_ecs_service" "calibration" {
  name            = "${var.project_name}-${var.environment}-ai-calibration-service"
  ...
  service_registries {
    registry_arn = aws_service_discovery_service.calibration.arn
  }
