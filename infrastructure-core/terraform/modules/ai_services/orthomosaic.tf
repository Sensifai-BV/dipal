# Orthomosaic Generation - SQS-triggered ECS Fargate service (0/1 scaling)
resource "aws_ecs_task_definition" "orthomosaic" {
  family                   = "${var.project_name}-${var.environment}-ai-orthomosaic-task"
  network_mode             = "awsvpc"
  requires_compatibilities = ["FARGATE"]
  cpu                      = var.orthomosaic_cpu
  memory                   = var.orthomosaic_memory
  ...
  container_definitions = jsonencode([
    {
      name      = "orthomosaic"
      image     = "${aws_ecr_repository.ai_services["orthomosaic-generation"].repository_url}:latest"
      essential = true

      command = [
        "uvicorn", "services.orthomosaic_generation.app.__main__:app",
      ...
      environment = [
        ...
        { name = "ORTHOMOSAIC_SERVICE_PORT", value = "8003" },
        ...
        { name = "ORTHOMOSAIC_DSM_RESOLUTION", value = "0.01" },
        { name = "ORTHOMOSAIC_DSM_OUTPUT_TYPE", value = "max" },
        { name = "ORTHOMOSAIC_DSM_RADIUS", value = "0.5" },
        { name = "ORTHOMOSAIC_PDAL_THREADS", value = "0" },
        { name = "ORTHOMOSAIC_FILL_MAX_DISTANCE", value = "100" },
        { name = "ORTHOMOSAIC_FILL_SMOOTHING_ITERATIONS", value = "0" },
        { name = "ORTHOMOSAIC_COG_COMPRESSION", value = "LZW" },
        { name = "ORTHOMOSAIC_COG_PREDICTOR", value = "2" },
        { name = "ORTHOMOSAIC_COG_BLOCKSIZE", value = "512" },
        { name = "ORTHOMOSAIC_COG_OVERVIEW_RESAMPLING", value = "AVERAGE" },
        { name = "ORTHOMOSAIC_ORTHO_RESOLUTION", value = "0.01" },
        { name = "ORTHOMOSAIC_ORTHO_INTERPOLATION", value = "bilinear" },
        { name = "GDAL_NUM_THREADS", value = "ALL_CPUS" },
      ]
      ...
      healthCheck = {
        command     = ["CMD-SHELL", "python3 -c \"import urllib.request; urllib.request.urlopen('http://localhost:8003/health/')\" || exit 1"]
        interval    = 30
        timeout     = 5
        retries     = 3
        startPeriod = 60
      ...
      logConfiguration = {
        ...
        options = {
          ...
          "awslogs-stream-prefix" = "orthomosaic"
        }
      }
    }
  ])
  ...
  tags = merge(var.tags, {
    Name      = "${var.project_name}-${var.environment}-ai-orthomosaic-task"
    Component = "AI-Services"
  })
}

resource "aws_ecs_service" "orthomosaic" {
  name            = "${var.project_name}-${var.environment}-ai-orthomosaic-service"
  cluster         = var.ecs_cluster_id
  task_definition = aws_ecs_task_definition.orthomosaic.arn
  desired_count   = 0
  launch_type     = "FARGATE"
  ...
  service_registries {
    registry_arn = aws_service_discovery_service.orthomosaic.arn
  }
